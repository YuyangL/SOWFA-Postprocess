...
for casename in casenames:
    ...
    linelabel = None if 'OneTurb' in casename else ('Average', 'Turb0', 'Turb1')
    ...
    list_y = (turb.times_selected,)*(turb.n_turbs + 1) if 'OneTurb' not in casename else (turb.times_selected,)
    ...
    for i0, property in enumerate(property_names):
        ...
        # xlim = (1e9, -1e9)
        # Go through turbines
        total = np.zeros(len(turb.times_selected))
        for i in range(turb.n_turbs):
            mean_property = property + '_Turb' + str(i)
            list_x.append(turb.data[mean_property + '_mean']/1000.)
            ...
            total += turb.data[mean_property + '_mean']/1000.
